...
import {
  Card,CardHeader,CardBody,  CardFooter,  Typography,  Input,Button,} from "@material-tailwind/react";
...
const [loading,setLoading]=useState(false);
  ...
  const validationSchema=Yup.object({
    ...
    password:Yup.string().required("Required").min('6','Must be at least 6 characters').matches(/^[a-zA-Z]+$/,"Password can only contain letters")
  })
      ...
      <Card className="w-96">
        ...
        <CardBody className="flex flex-col gap-4">
          <form onSubmit={handleSubmit}>
            ...
            <div>{formik.touched.email&& formik.errors.email&&<Typography variant="small" color="red">
                {formik.errors.email}
              </Typography>}</div>
            <div className="mb-2 mt-4">
              <Input name="password" type="password" label="Password" size="lg" {...formik.getFieldProps("password")} />
            </div>
            <div>{formik.touched.password&& formik.errors.password&&<Typography variant="small" color="red">
